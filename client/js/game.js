...
function renderCommand() {
  ...
  $('#hud-stats').innerHTML = [stat('钱包', isk(ch.credits)), stat('当前船', escapeHtml(ch.ship?.zh || ch.ship?.name || '-')), stat('状态', ch.expedition?.state || 'idle'), stat('货舱', `${used.toFixed(1)} / ${cap} m³`), stat('击杀/损失', `${ch.stats?.kills || 0} / ${ch.stats?.losses || 0}`), stat('Dogma', combatMeta?.dogma?.version || '-')].join('');
...
function renderHangar(){ const ch=state.character; $('#ship-card') && ($('#ship-card').innerHTML=`<h2>${escapeHtml(ch.ship?.zh||ch.ship?.name)}</h2><div class="stat-cards">${stat('DPS',Math.round(ch.ship?.stats?.dps||0))+stat('护盾',Math.round(ch.ship?.stats?.shield||0))+stat('货舱',Math.round(ch.ship?.stats?.cargo||0))+stat('扫描',Math.round(ch.ship?.stats?.scan||0))}</div>`); $('#fitting-list') && ($('#fitting-list').innerHTML=(ch.ship?.fittedModules||[]).map(m=>`<div class="mini-card"><b>${escapeHtml(m.zh||m.name)}</b><span>${m.slot} ${escapeHtml(JSON.stringify(m.effects||{}))}</span><button data-unfit="${m.instanceId}">卸下</button></div>`).join('')||'<p class="muted">没有装配。</p>'); }
...
window.addEventListener('DOMContentLoaded',async()=>{setupSocket();bindActions();animate();try{await loadState();}catch(err){if(!['home','login','register'].includes(page))toast(err.message);}});
