...
function mutableCargo(character) {
  if (!Array.isArray(character.cargo)) character.cargo = [];
  return character.cargo;
}

function mutableWarehouse(character) {
  if (!character.warehouse) character.warehouse = { capacity: 50000, items: [], reserve: new Map() };
  if (!Array.isArray(character.warehouse.items)) character.warehouse.items = [];
  return character.warehouse.items;
}

function addLog(character, line) {
  if (!Array.isArray(character.expedition.log)) character.expedition.log = [];
  character.expedition.log.unshift(`${new Date().toLocaleTimeString('zh-CN', { hour12: false })} ${line}`);
  character.expedition.log = character.expedition.log.slice(0, 30);
}

function cargoCapacity(character, stats) {
  const activity = character.autopilot?.activity;
  if (activity === 'mining' && stats.oreHold > 0) return stats.oreHold;
  return stats.cargo;
}

function hpPercent(character, stats) {
  const hp = character.expedition.site?.hp;
  if (!hp) return 1;
  const current = Number(hp.shield || 0) + Number(hp.armor || 0) + Number(hp.hull || 0);
  const max = Number(stats.shield || 0) + Number(stats.armor || 0) + Number(stats.hull || 0);
...
function applyDamage(character, amount) {
  ...
  const shieldHit = Math.min(hp.shield, dmg);
  hp.shield -= shieldHit;
  dmg -= shieldHit;
  const armorHit = Math.min(hp.armor, dmg);
  hp.armor -= armorHit;
  dmg -= armorHit;
  const hullHit = Math.min(hp.hull, dmg);
  hp.hull -= hullHit;
  dmg -= hullHit;
  character.expedition.hazard = Number(character.expedition.hazard || 0) + amount / 500;
  return hp.hull <= 0;
}

function resetExpedition(character, state = 'idle') {
  character.expedition.state = state;
  character.expedition.progress = 0;
  character.expedition.enemyHull = 0;
  character.expedition.hazard = 0;
  character.expedition.site = undefined;
}
...
async function lootPool(activity) {
  const kindMap = {
    mining: ['ore'],
    ratting: ['salvage', 'commodity', 'mineral'],
    combat: ['salvage', 'commodity', 'module'],
    relic: ['salvage', 'data'],
    data: ['data', 'salvage'],
    hauling: ['commodity', 'mineral']
  };
...
async function generateLoot(character, system, site, stats, rng) {
  ...
  for (let i = 0; i < rolls; i += 1) {
    ...
    loot.push({
      typeId: String(pick.typeId),
      name: pick.name,
      zh: pick.zh || pick.name,
      kind: pick.kind,
      quantity: qty,
      volume: Number(pick.volume || 0.01),
      basePrice: Number(pick.basePrice || 1),
...
function transferCargoToWarehouse(character) {
  ...
  for (const stack of cargo) {
    mergeStack(warehouse, stack);
    count += Number(stack.quantity || 0);
    value += Number(stack.quantity || 0) * Number(stack.basePrice || 1);
  }
  character.cargo = [];
  return { count, value };
}
...
async function sellExcess(character, system) {
  ...
  for (const stack of warehouse) {
    ...
    const amount = Math.round(price * available);
    sold += available;
    value += amount;
  }
...
async function tickIndustry(character, now, io) {
  ...
  for (const job of readyJobs) {
    ...
    mergeStack(warehouse, {
      typeId: String(job.productTypeId),
      name: type?.name || job.productName,
      zh: type?.zh || job.productName,
      kind: type?.kind || 'item',
      quantity: Number(job.output?.quantity || job.runs || 1),
      volume: Number(type?.volume || 0.01),
...
async function tickStep(character, dt, now, io) {
  ...
  if (!character.autopilot?.enabled) {
    character.locationState = character.locationState === 'space' ? 'space' : 'docked';
    character.skillpoints += dt * 0.015;
    return;
  }
  ...
  if (state === 'idle') {
    await startNewSite(character, system, stats, rng);
    return;
  }

  if (state === 'repairing') {
    character.expedition.progress += dt * 2;
    if (character.expedition.progress >= 100) {
      resetExpedition(character, 'idle');
      character.locationState = 'docked';
      addLog(character, '维修完成，等待下一轮出站。');
    }
    return;
  }

  if (!character.expedition.site) {
    resetExpedition(character, 'idle');
    return;
  }
  ...
  if (extractionReason && !['extracting', 'looting'].includes(state)) {
    character.expedition.state = 'extracting';
    character.expedition.progress = 0;
    addLog(character, `${extractionReason}，开始撤离。`);
    return;
  }
  ...
  if (state === 'scanning') {
    ...
    if (character.expedition.progress >= site.scanNeed) {
      character.expedition.state = 'warping';
      character.expedition.progress = 0;
      addLog(character, `锁定 ${site.name}，跃迁进场。`);
    }
    return;
  }
  ...
  if (state === 'warping') {
    ...
    if (character.expedition.progress >= 40) {
      ...
      addLog(character, peaceful ? `进入 ${site.name}，开始作业。` : `遭遇守卫，进入战斗。`);
    }
    return;
  }

  if (state === 'fighting') {
    const playerDamage = Math.max(1, stats.dps) * dt * (0.85 + rng() * 0.3);
    character.expedition.enemyHull = Math.max(0, Number(character.expedition.enemyHull || site.enemyEhp) - playerDamage);
    const incoming = Math.max(0, site.enemyDps * dt * (0.6 + rng() * 0.8) - stats.warpStability * 0.4);
    const destroyed = applyDamage(character, incoming);
    if (destroyed) {
    ...
    if (character.expedition.enemyHull <= 0) {
      character.stats.kills += 1;
      character.expedition.state = 'looting';
      character.expedition.progress = 0;
      addLog(character, '清理守卫，开始搜刮/采集。');
    }
    return;
  }
  ...
  if (state === 'looting') {
    ...
    if (character.expedition.progress >= site.lootNeed) {
      ...
      for (const stack of loot) {
        const used = cargoVolume(cargo);
        const availableM3 = Math.max(0, cap - used);
        const maxQty = stack.volume > 0 ? Math.max(0, Math.floor(availableM3 / stack.volume)) : stack.quantity;
        const qty = Math.min(stack.quantity, maxQty);
        if (qty <= 0) continue;
        const accepted = { ...stack, quantity: qty };
        mergeStack(cargo, accepted);
  ...
  if (state === 'extracting') {
    ...
    if (Number(character.expedition.hazard || 0) > 1.25 + Number(stats.warpStability || 0) * 0.12) {
      const destroyed = applyDamage(character, site.enemyDps * dt * 2.5);
      if (destroyed) {
        character.stats.losses += 1;
        character.cargo = [];
        character.locationState = 'docked';
        resetExpedition(character, 'repairing');
        addLog(character, '撤离失败，遭遇反跳并损失舰船。');
        await pushEvent({ scope: 'global', characterId: character._id, systemId: system.systemId, severity: 'danger', title: '撤离失败', message: `${character.name} 在 ${system.name} 被反跳击毁。` }, io);
        return;
      }
    }
    if (character.expedition.progress >= 65) {
      ...
      if (sold.value > 0 || transferred.value > 100000) {
        await pushEvent({ scope: 'system', characterId: character._id, systemId: system.systemId, severity: 'success', title: '搜打撤成功', message: `${character.name} 从 ${system.name} 带回约 ${Math.round(transferred.value)} ISK 货物。` }, io);
      }
      resetExpedition(character, character.autopilot?.loop ? 'idle' : 'idle');
    }
  }
}
...
export async function tickCharacter(character, now = new Date(), io = null) {
  ...
  while (elapsed > 0) {
    const step = Math.min(STEP_SECONDS, elapsed);
    await tickStep(character, step, now, io);
    elapsed -= step;
  }
...
export async function tickCharacterById(characterId, io = null) {
  const character = await Character.findById(characterId);
  if (!character) return null;
  return tickCharacter(character, new Date(), io);
}
...
export function publicCharacter(character) {
  ...
  return {
    ...
    currentSystemId: obj.currentSystemId,
    homeSystemId: obj.homeSystemId,
    locationState: obj.locationState,
    ship: obj.ship,
    cargo: obj.cargo || [],
    warehouse: obj.warehouse || { items: [], capacity: 0 },
    skills: obj.skills,
    autopilot: obj.autopilot,
  ...
  for (const fleet of fleets) {
    ...
    if (progress >= 1 || (fleet.readyAt && new Date(fleet.readyAt) <= now)) {
      ...
      for (const memberId of memberIds) {
        ...
        character.credits += shared;
        character.stats.totalEarned += shared;
        character.walletJournal.unshift({ at: now, type: 'fleet', amount: shared, note: `${fleet.name} 舰队分红` });
        const pick = chooseWeighted(pool, rng)?.item;
      ...
      fleet.lootPool = { credits: baseReward, items: [] };
      fleet.log.unshift(`目标完成，舰队分红 ${baseReward} ISK。`);
      await fleet.save();
      ...
      await fleet.save();
      io?.to('global').emit('fleet:update', fleet.toObject());
    }
  }
}
...
export async function startGameLoop(io, { tickMs = 5000 } = {}) {
  ...
  const loop = async () => {
    ...
    try {
      const now = new Date();
      const chars = await Character.find({ 'autopilot.enabled': true }).limit(300);
      for (const character of chars) await tickCharacter(character, now, io);
      await tickFleets(now, io);
    } catch (error) {
...
export async function startIndustryJob(character, blueprintTypeId, runs = 1) {
  ...
  for (const material of bp.materials) {
    const have = warehouse.find(s => String(s.typeId) === String(material.typeId));
    if (!have || Number(have.quantity || 0) < Number(material.quantity || 0) * runs) {
      throw new Error(`材料不足：${material.name || material.typeId}`);
    }
  }
  for (const material of bp.materials) {
    const stack = warehouse.find(s => String(s.typeId) === String(material.typeId));
    stack.quantity -= Number(material.quantity || 0) * runs;
  }
  character.warehouse.items = warehouse.filter(s => Number(s.quantity || 0) > 0);
  const seconds = Math.max(10, Number(bp.time || 60) * runs / (1 + Number(character.skills?.industry || 1) * 0.04));
  const readyAt = new Date(Date.now() + seconds * 1000);
  const job = await IndustryJob.create({
    ...
    output: { typeId: bp.productTypeId, quantity: Number(bp.quantity || 1) * runs },
    cost: Math.round(seconds * 8)
  });
  await character.save();
  return job;
}
